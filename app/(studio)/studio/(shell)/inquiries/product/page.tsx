...
/**
 * /studio/inquiries/product
 *
 * Only `PRODUCT` enquiries. The filter is a database predicate rather than a client-side one, so a role that may not read an enquiry never receives it to filter.
 *
 ...
 */
export const metadata = studioMetadata('/studio/inquiries/product')

export default async function Page() {
  await requirePermission('inquiries.read')
  const client = await createClient()
  const [rows, products] = await Promise.all([
    listInquiriesForStudio(client, { kind: 'PRODUCT' }),
  ...
  return (
    <StudioPage path="/studio/inquiries/product">
      <Stack gap={8}>
