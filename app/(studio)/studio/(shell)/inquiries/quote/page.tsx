 ...
 *
 * Only `QUOTE` enquiries. The filter is a database predicate rather than a client-side one, so a role that may not read an enquiry never receives it to filter.
 *
...
export default async function Page() {
  ...
  const [rows, products] = await Promise.all([
    listInquiriesForStudio(client, { kind: 'QUOTE' }),
  ...
  return (
    <StudioPage path="/studio/inquiries/quote">
      <Stack gap={8}>
