...
/**
 * /studio/media/images — FEAT §13's Images section.
 *
 * `kind = 'IMAGE'` and not "everything that renders": BRAND assets are images too and have their
 * own section, because the questions asked of a logo (is it the current one, does it have alpha)
 * are not the questions asked of a photograph.
 */
export const metadata = studioMetadata('/studio/media/images')

const PATH = '/studio/media/images'

// The 23 manifest folders. An image belongs wherever its subject does, so this is the
// one kind where the full editorial tree is the right list.
const IMAGE_FOLDERS = MANIFEST_FOLDERS
  ...
  return (
    <StudioPage path={PATH}>
      ...
      <PermissionGate role={session.role} permission="media.write">
        <MediaUploader kind="IMAGE" folders={IMAGE_FOLDERS} saveAction={saveUploadedAssetAction} />
      </PermissionGate>
      <MediaLibrary
        path={PATH}
        assets={assets}
