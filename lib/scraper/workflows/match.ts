...
/**
 * `pg_trgm.similarity` LIVES IN `core/similarity.ts` NOW, and is re-exported here.
 *
 * It moved when Phase 29's materiality rules needed the same function for `title` and
 * `description`: `analytics/` importing from `workflows/` would be the dependency arrow pointing
 * the wrong way, and a second copy of a similarity function is two functions that disagree in the
 * argument nobody expected. Re-exported rather than relocated silently so that every import that
 * already named this module keeps working.
 */
export { trigramSet, trigramSimilarity } from '../core/similarity'
