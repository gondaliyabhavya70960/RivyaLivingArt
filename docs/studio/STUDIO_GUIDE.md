...
**THERE ARE NO SCREENSHOTS, AND THEY CANNOT BE TAKEN IN THIS REPOSITORY.** Phase 46 asks for these
procedures *"with screenshots taken against fixture data"* (`docs/project/phases/PHASE-39-46.md`,
Phase 46 scope) and this section has none — a gap named here rather than left for the owner to
discover. The reason is mechanical rather than editorial: signing into the Studio needs Supabase
Auth, and the local harness (`scripts/db/local-rest.mjs`) is PostgREST alone, so there is no auth
server to authenticate against and no browser can reach a Studio page in this container at all. It is
the same wall that makes every Studio browser specification skip behind `STUDIO_STORAGE_STATE` — 156
of them at the last full run (`tests/e2e/studio-system.spec.ts`), recorded in `docs/ops/TESTING.md`
§13. They therefore have to be captured somewhere a person can sign in: a preview deployment pointed
at the Phase 42 fixture database (`docs/ops/TESTING.md` §2), never from a shell here. When they are
captured they are documentation images under `docs/` — they do not enter `media_assets`, they never go
to Cloudinary, and per the phase's own media note **no screenshot may show a real enquirer's personal
data or any environment value**, which is the second reason for the fixture database rather than the
real one. **The thirteen procedures below are complete without them:** each names its route, the
control to press and the field it writes, which is what a reader follows. An illustration is missing;
a step is not.
...
**This list and the generated backlog are two different instruments, and neither replaces the other.**
The table below is hand-written and its subject is *this document*: sentences here that describe a
control accurately while implying something about the real business that only the owner can settle.
The machine-checked list is the **Owner-verification backlog** in
`docs/content/INITIAL_CONTENT_INVENTORY.md`, generated by `npm run content:verification-report`
(`scripts/content/build-verification-report.ts`) from the database itself: every row still carrying
`owner_verification = OWNER_VERIFICATION_REQUIRED`, grouped by the surface it appears on, with the
Studio path that resolves it and one sentence saying what the owner is being asked to confirm. Those
rows are a **publication blocker** rather than a reading list — the Phase 08 triggers refuse them
`PUBLISHED` — and the same declaration behind the report (`lib/cms/verification-backlog.ts`) feeds the
*Outstanding owner verifications* card on the `/studio` overview, which counts the live database
rather than the seeded one, because a list in a Markdown file is not where an owner looks.

**The backlog's contents are deliberately not repeated here.** It is generated, its number moves with
the database, and a figure hand-copied into this guide would be stale by the next seed. Read the two
together: the table below is what *this document* claims; the backlog is what *the database* cannot
publish.
...
| Whether the owner can actually perform the ten routine operations unaided | §19 | The capability boundary is a claim until somebody tries it. The ten operations are written down once, with their confirmations and reversals, in `docs/ops/TESTING.md` §14.3, and the record they fill is §14.4; the verdict row is `DESIGN_SYSTEM.md` §19.1 row 8. Nobody has performed them |
| Whether the handover session happened, and on what date | §19 → *The handover session* | A meeting is a fact about the world, and D10 reserves those to the owner. No document can assert it took place |
...
**THE BOUNDARY IS A CLAIM, AND THE CLAIM HAS NOT BEEN DRY-RUN.** Phase 46 does not pass on this table
being written well: its risk table requires the owner to perform ten routine changes unaided first,
and the result to be recorded as *ten of ten* or as a precise list of what needed an engineer. That
dry run is FEAT §49 question 8 — `docs/design/DESIGN_SYSTEM.md` §19.1 row 8, which today reads
`OWNER_VERIFICATION_REQUIRED` with no reviewer and no date — and the ten operations themselves, each
with its Studio path, the field it touches, how to confirm it on the public site and how to put it
back, are written down once in `docs/ops/TESTING.md` §14.3, with the record they fill in §14.4.
**State today: not run.** No count is written here, because the only honest count is the one the owner
produces, and `TESTING.md` §14.3 also warns that O10 is expected to save and show nothing — so *ten of
ten* is not the outcome to expect.

**Three rows below are already contradicted by that section, and they are defects rather than a
difference of opinion** — recorded at the boundary so a reader meets them where the promise is made:

| The row above that promises it | What the repository holds today |
|---|---|
| Publishing a category, so its route exists | `saveCategoryAction` in `app/(studio)/studio/(shell)/catalog/actions.ts` writes name, subtitle, description, order, hero image and the two SEO fields, and never a content status; there is no `publishCategoryAction` anywhere in the repository. All seven categories stay `DRAFT` and their routes 404 until an engineer adds the control |
| FAQ questions, answers, categories and order | `app/(studio)/studio/(shell)/content/faqs/page.tsx` is a `StudioPage` stub whose own comment says Phase 08 replaces it (§3 gives the same owning phase), so `/faq` cannot be fixed by an editor today |
| Contact details, the WhatsApp number, and both WhatsApp message templates | `app/(studio)/studio/(shell)/system/settings/page.tsx` is a `StudioPage` stub whose own comment says Phase 20 replaces it (§3 again). The WhatsApp number is an engineer change today, which `TESTING.md` §14.3 calls the single most surprising item on the list for an owner |

Each of the three is the designed surface described in §13.8 and §9.3 under the stub policy in §3 —
...
Phase 46 ends in a walkthrough with the owner, and this subsection is where it is recorded. Four
things are covered, in this order, because each one is the context for the next:

| # | What the session covers | Where it is written down |
|---|---|---|
| 1 | The capability boundary — both tables above, and the three rows that are designed rather than shipped | §19 |
| 2 | The runbooks, followed by the owner rather than demonstrated to them | §16 |
| 3 | The owner-verification backlog: why nothing in it can publish, and where the count lives | §17, `docs/content/INITIAL_CONTENT_INVENTORY.md`, the `/studio` overview card |
| 4 | The incident path: what to read, in what order, and what to send when asking for an engineer | `docs/ops/DEPLOYMENT.md` §10, then runbook 13 |

**State: not held.** `OWNER_VERIFICATION_REQUIRED`, and it stays that way until the owner writes the
date here themselves. Whether a meeting happened is a fact about the world, and D10 puts facts about
the world outside this repository's reach: a document asserting its own handover is the single
cheapest sentence in this guide to write and the one with the least evidence behind it. It is also why
this document's front matter now reads `owner_verification: OWNER_VERIFICATION_REQUIRED` — the same
interlock `docs/ops/TESTING.md` and `docs/design/DESIGN_SYSTEM.md` carry, for the same reason
(`DESIGN_SYSTEM.md` §19.2 states it as a convention): a guide claiming it needs nothing from the owner
while three of its own statements are unconfirmed would be wrong in the front matter as well as in the
table.

| Field | Filled by | Value |
|---|---|---|
| Date held | The owner | — |
| Who attended | The owner | — |
| What the owner could not do afterwards | The owner, in their own words | — |
