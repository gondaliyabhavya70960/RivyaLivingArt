...
| 2 | Does large furniture visually dominate? | `/` and `/large-format`, all eight FEAT §45 widths | Measured. The hero media box is **617 px = 69 % of 900** at 1440 and **693 px = 82 % of 844** at 390; across the eight widths it runs 91 · 69 · 61 · 49 · 37 · 91 · 82 · 76 %. It is a 21:9 `AspectBox`, so its height is a function of WIDTH and never of viewport height — the ≥ 70 % condition is met or missed by accident of viewport aspect, which is the finding rather than the one missing point. The scale-reference-above-the-fold clause and the "no large-format piece smaller than a décor item" clause are **not yet observed**; a row takes the worst verdict among its measured clauses and names the rest. **The dominance clause now passes, re-measured 2026-09-12.** `--rv-hero-min-h` (§5.1.1) gives the first section of a page a viewport-relative floor of 76 svh. Measured again in Chromium against a local build over the full harness — Postgres, PostgREST and `next dev` — the hero media box runs **91 · 76 · 76 · 76 · 76 · 91 · 82 · 76 %** across the eight widths on `/` and the same on `/large-format`, against 91 · 69 · 61 · 49 · 37 · 91 · 82 · 76 % before: the worst case moves from 37 % to 76 % and every width clears ≥ 70 %. `/about` reads 76 % at all eight. **The row stays `FAIL` because the question has three clauses and only this one has been observed** — the scale-reference-above-the-fold clause and the "no large-format piece smaller than a décor item" clause are still unmeasured, and a row takes the worst verdict among its clauses | `FAIL` | — (measured; see §19.3) | 2026-09-12 |
| 3 | Can the visitor feel resin, wood, light and surface? | `/about` material palette, `/process`, product galleries; delivery presets and `sizes` read statically, because the browser harness serves committed fixture derivatives and cannot judge texture | Seventeen `sizes` strings declare a `(min-width: 640px)` switch this design system does not have — `--breakpoint-sm` is 26.875rem (430 px) and `--breakpoint-md` is 48rem (768 px) — so the browser is told the layout changes where it does not. `media_crops` focal points were written by the Studio and read by no public renderer — **since fixed**: `loadPageMedia` now loads them with `cropsForAssets` and `BlockImage` resolves one per ratio, so a crop reaches the `src` and every `srcSet` rung. The material palette forces `mobileRatio="4:5"` on a macro study | `FAIL` | — (measured; see §19.3) | 2026-09-12 |
...
| 6 | Can users Enquire · Customize · Request Quote · Request Consultation · Commission · Continue to WhatsApp? | Every conversion surface at 390 px, plus `tests/e2e/inquiry-conversion.spec.ts` in both directions | Six named affordances resolve to **one** form reached by two URLs. `ProductInquiryRail` returns the same `/contact?product=<slug>&type=product` for "Ask About This Piece" and "Request a Quote", and `type` is read by nothing; both file `kind = 'PRODUCT'`. `QUOTE` and `CONSULTATION` are real enum values with real schemas, real WhatsApp templates and two Studio inbox views that can never receive a row. `Customize` is gated on a flag that ships off, behind a block type nothing seeds. Persistence-before-redirect **passes** structurally. The pass condition's own "two clicks" is unsatisfiable for affordance six — see the note below this table. **The routing has since been fixed**: the rail sends each affordance to its own `?type=`, and `InquiryForm` reads it through a closed allowlist, so `QUOTE` reaches `/studio/inquiries/quote` and `CONSULTATION` is reachable by URL. `CONSULTATION` still has no BUTTON — SEED §31 fixes the seven action labels and none names a consultation, so an affordance for it needs copy only the owner can write. The verdict stands at `FAIL` until somebody re-runs the 390 px pass and signs it, per §19.2 | `FAIL` | — (measured; see §19.3) | 2026-09-12 |
| 7 | Does mobile feel intentionally designed? | Every tier A and B route at 430, 390 and 360 | No route scrolls sideways at any of the eight widths, and `rv-hit-44` holds every touch target. Against that: there is no sticky conversion affordance anywhere, so on `/product/<slug>` at 390 the two calls to action sit at y = 997 and 1053 of a 1928 px document, outside the thumb zone on every phone; the journal card's `h2` truncates at 390 and 360; and the mobile crops the Studio can write are now read: `ResponsiveMedia`'s mobile half resolves its own crop at `mobileRatio`, which is the half of this question that was missing. **The sticky affordance now exists**: `.rv-sticky-actions` (§7.10) carries the product rail and the enquiry submit row at the bottom of the viewport below 768px, so neither is below the fold on a phone; the geometry wants re-measuring in a browser. **The truncated heading is fixed**: `ArticleCard` no longer clamps its title at any width — the excerpt is already `hidden sm:block`, so a clipped title left a visitor choosing between three dots and a date, and even rows are worth less than a legible one. The unread crops are unchanged. The verdict stands at `FAIL` until somebody re-runs the 390 px pass and signs it, per §19.2 | `FAIL` | — (measured; see §19.3) | 2026-09-12 |
