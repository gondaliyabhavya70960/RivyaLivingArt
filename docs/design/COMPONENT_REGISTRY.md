...
| RC-245 | `SectionRail` | navigation | 46 | BUILT | index only — server; the page-section index (§7.15, A48). A fixed 56px column at `xl` and above listing each band that carries an `eyebrow`, numbered over what it lists rather than over the page, so a band withheld for owner verification leaves no hole in the numbering. NO active-state highlight and that is the decision: tracking the visible band needs either a scroll handler — an island on all sixteen CMS routes — or a `timeline-scope` naming every section, which is static CSS against a section count that is data. Shown at `xl` because the container's own margin is already wider than the rail there (97px at 1280); an earlier version showed it at `lg` and bought the space with container padding, which double-inset the copy to 240px at 1440 |
