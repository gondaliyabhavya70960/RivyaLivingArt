...
| **SmoothUI — Animated Tabs** | `https://smoothui.dev/docs/components/animated-tabs` → `packages/smoothui/components/animated-tabs/index.tsx` | Studio editor / media tabs | `NOT_ADOPTED` — **source read, see §5.2** | **It is a tab STRIP, not a tabs widget.** 165 lines rendering `role="tablist"` and `role="tab"` with roving `tabIndex`, Arrow/Home/End and `useReducedMotion` — and **zero `aria-controls` and zero `tabpanel`**: it renders no panels and associates a tab with nothing. Adopting it would LOSE the panel association RC-203 already has. It also declares `motion@^12.23.25`, which would be this project's first animation runtime. |
...
### 5.2 SmoothUI Animated Tabs, read at source — and what was built instead (amendment A51)

§5.1 deferred this one on verification grounds and named it "the one candidate worth reopening". The
owner asked for it to be reopened on 2026-09-13, so the source was located and read. **The verdict
changed its basis entirely and stayed NOT_ADOPTED**, for a reason no amount of architectural
reasoning would have found.

**Locating it took eleven 404s and is worth recording.** `smoothui.dev` is unreachable from this
session and GitHub's tree API is scoped to this account's own repositories, so the file could only be
found by guessing paths. `pnpm-workspace.yaml` is what gave it up: the repository is a monorepo whose
components are one workspace package each, under `packages/smoothui/components/*`. The file is
`packages/smoothui/components/animated-tabs/index.tsx`.

| Read from source | |
|---|---|
| Package | `@repo/animated-tabs`, version `0.0.0`, **`license` field absent** — the MIT grant comes from the repository `LICENSE`, not from the package |
| Dependencies | `motion@^12.23.25`, `@repo/shadcn-ui` (workspace), react/react-dom 19 |
| Size | 165 lines, one default export |
| Has | `role="tablist"`, `role="tab"`, `aria-selected`, roving `tabIndex`, `ArrowLeft`/`ArrowRight`/`Home`/`End`, `useReducedMotion`, three variants |
| **Lacks** | **`aria-controls` and `role="tabpanel"` — zero occurrences.** It renders a `<div>` of `<button>`s and no panels at all |

**That is the finding.** It is a beautifully animated tab STRIP, and the APG tabs pattern is a strip
**plus** panels associated by `aria-controls`/`aria-labelledby`. RC-203 has both; this has the first
half. Replacing RC-203 with it would have traded a complete, tested accessibility contract for a
sliding underline — a straight regression, and one that a licence check and a bundle-size check
would both have waved through.

**What was built instead, and it is first-party.** The one thing SmoothUI has that RC-203 did not is
the travelling bar, so that was implemented in `components/patterns/Tabs/index.tsx` behind
`indicator="slide"`, opted into by Studio and **off by default because `/product/[slug]` renders the
same component**. No dependency was added: the bar is `translateX` + `scaleX` on a 1px element,
measured with a `ResizeObserver` that watches both the tab and the strip. Per the brief — *"A
first-party approximation must not be reported as an imported library component"* — **this is
recorded as first-party work with SmoothUI as the visual reference, and nothing is imported.**

**A defect the work found in itself.** The first version treated any measurement as sufficient. A
box reports `0` while it is inside a `display: none` ancestor or before a webfont resolves, so it
drew a bar at `scaleX(0)` — invisible — *and* turned the per-tab underline transparent, leaving the
strip with no selection shape and only the ink step to carry it, which WCAG 1.4.1 does not accept.
The guard is `w > 0`, and `Tabs.test.tsx` fails without it.
...
| RC-245 | `SectionRail` | navigation | 46 | BUILT | index only — server; the page-section index (DESIGN_SYSTEM §7.15, A48). A fixed 56px column at `xl` and above listing each band that carries an `eyebrow`, numbered over what it lists rather than over the page, so a band withheld for owner verification leaves no hole in the numbering. NO active-state highlight and that is the decision: tracking the visible band needs either a scroll handler — an island on all sixteen CMS routes — or a `timeline-scope` naming every section, which is static CSS against a section count that is data. Shown at `xl` because the container's own margin is already wider than the rail there (97px at 1280); an earlier version showed it at `lg` and bought the space with container padding, which double-inset the copy to 240px at 1440 |
...
| RC-356 | `AccessibilityPanel` | usability | 41 | BUILT | index only — the text alternative and the decorative flag edited together on `/studio/media/all/[assetId]`, because `0390`'s CHECK is about both. Client Component so the quality warnings appear while somebody types, in a `role="status"` region rather than an assertive one; the warnings are advice and never block a save |
| RC-357 | `SecurityPosture` | usability | 41 | BUILT | index only — the Security section of `/studio/system/environment`: policy enforced or collecting, each salt and the cron secret as set or not set, the header names, the two documented policy exceptions. State only — `readSecurityPosture()` returns booleans and counts, so there is no value on the surface to leak |
| RC-358 | `DataRequestPanel` | usability | 41 | BUILT | index only — a subject access request and an erasure, in that order: identify, preview, then act. Download and Erase stay disabled until a preview has matched, erase asks for a typed word, and the reference codes go back with the erasure so the action can refuse a set that moved. Holds a count and reference codes, never a contact detail |
| RC-359 | `CropEditor` | product understanding | 43 | BUILT | index only — one editor-chosen crop per (asset, D6 ratio), so one 4800px master serves a 21:9 hero and a 9:16 phone banner without a second generation. A keyed child form per ratio rather than an effect that rewrites six fields; warns on a box outside the source and on a box that drifts from its nominal shape, and disables the save only for the first. Typed pixels rather than a drag handle, stated as a first version |
| RC-360 | `AltTextQueue` | usability | 43 | BUILT | index only — every asset whose text alternative still needs a person, ordered bound-first then truncated-first then by warning count, because an alphabetical list of 250 would spend the first hour on an unbound macro texture. A Server Component: the editing happens on the asset page where the picture is large enough to write about |
| RC-361 | `BrandFormats` | usability | 43 | BUILT | index only — the four owner-supplied brand assets and the format each must arrive in, stated BEFORE a file is chosen. The formats are read from `BRAND_SLOTS`, the same table the Phase 41 validator enforces, so the panel cannot promise something the upload would refuse |
| RC-362 | `EnvironmentRibbon` | usability | 44 | BUILT | index only — a band across the top of every non-production deployment naming the environment and the commit. Renders null only for the literal `production`, so a missing `VERCEL_ENV` shows it: the safe direction is to say which environment this is rather than assume it is the real one. A Server Component, so it costs no JavaScript and cannot be dismissed |
| RC-363 | `BuildPanel` | usability | 44 | BUILT | index only — what is actually deployed: commit, branch, build time, environment, and the migration-state row that compares what this build carries against what the database records. Identifiers only, never a value — the D8 distinction the whole environment page rests on. BEHIND is a warning; AHEAD is `info`, because it is the safe half of an expand/contract window |
...
| Purpose | usability |
| Adaptation | Two indicator branches. `indicator="static"` (default, and what every public route gets) draws the per-tab underline in `--rv-ink-accent` and transitions COLOUR at `--rv-duration-quick` — §4.2 LIGHT. `indicator="slide"` (amendment A51, opted into by Studio) draws ONE bar that travels: `translateX` + `scaleX` on a 1px element at `--rv-duration-base` — §4.2 FORM. Panel swap is `--rv-duration-instant` in both |
...
| Accessibility | `role="tablist"`/`tab`/`tabpanel`, roving tabindex, `aria-selected`, `aria-controls`; arrow keys move and activate, `Home`/`End` jump; panels are `tabindex="-1"`; the travelling bar is `aria-hidden` and does not travel under reduced motion. **Selection always has a SHAPE (WCAG 1.4.1):** the bar draws only once a measurement with a non-zero width exists, and until then — the server render, JavaScript off, a `display: none` ancestor, a webfont still resolving — the per-tab underline draws instead |
| Reviewed on | — |
| Reviewer | UNASSIGNED |
| Verdict | FIRST_PARTY |
...
| Performance | **Not in any route's initial bundle.** `HeroSection` reaches it through `next/dynamic`, for the reason RC-216 does: `components/sections/registry.ts` imports every renderer, so a static import charged this island to all sixteen CMS routes — `/privacy` and `/terms` among them, neither of which has ever rendered a hero. Freeing it and RC-215 took every public route from seven islands to five. It imports `MediaVideo`, which is therefore inside this island's bundle rather than a boundary of its own. Nothing is fetched until every gate passes; the video is `preload="none"` and its poster is the still already on screen |
...
| Performance | One `IntersectionObserver` for all four stages, not one each. **Not in any route's initial bundle:** `MaterialStorySection` reaches it through `next/dynamic`, for the reason RC-216 does. Deferring it costs nothing because the stages are server-rendered as `children` and the island only adds a `data-active` attribute — the unenhanced page is already the finished one. It renders no media of its own |
