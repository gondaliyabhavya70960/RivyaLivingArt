...
- [x] Migrations 0001–0008 apply cleanly to an empty database and are forward-only.
      *Verified: `npm run db:reset` against PostgreSQL 16.13, 8/8 applied. `db:check-migrations`
      enforces contiguous numbering.*
- [x] Every table listed above exists with the D5 common columns and the four seed columns.
      *Verified by `db:check-schema`, which reads `information_schema.columns`. Note it asserts
      **five** Tier C columns, not four: DATA_MODEL §1.2 adds `owner_edited`. `media_assets` is
      Tier A+B only — no seed module writes it.*
- [x] RLS is enabled on every table in `public` with no permissive policy yet (proved by a query
      against `pg_tables.rowsecurity`).
      *Verified: all ten tables `relrowsecurity = true`, zero policies. Proved to bite by
      disabling RLS on one table and re-running.*
- [x] `lib/supabase/database.types.ts` is generated and drift-checked in CI.
      *Generated by `scripts/db/gen-types.mjs` (amendment A4·c — the Supabase CLI needs Docker).
      `db:check-types` regenerates and diffs; determinism proved by generating twice and
      comparing bytes.*
- [x] `lib/supabase/admin.ts` starts with `import 'server-only'` and is imported nowhere outside
      its allowlist.
      *Verified: first line is `import 'server-only'`; nothing imports it yet. An ESLint
      `no-restricted-imports` rule now names the allowlist and was proved to bite.*
- [x] No `.from(` outside `lib/supabase/repositories/**`.
      *`db:check-data-layer`, proved both to bite on a real violation and NOT to fire on `.from(`
      inside a comment or string — the false positive that would make it ignored.*
- [x] `npm run seed:content` is proven idempotent and proven not to overwrite an owner edit.
      *7 inserted / then 0 inserted + 7 updated / then after an owner edit 1 skipped with the
      edit intact. Also proved re-seeding does not un-publish a published row (rule 7), and that
      `--force` refuses to run without `--only`.*
- [x] `supabase/migrations/**` contains no content-row inserts.
      *`db:check-migrations`, proved to bite.*
- [x] `price_state` holds exactly its three Phase 03 values, and the `FIXED` / `price_minor`
      deferral is recorded here and in `DATA_MODEL.md` against Phase 14 — so a later reader finds
      a decision, not a gap.
      *Verified: enum is `STARTING_FROM REQUEST_QUOTE PRICE_ON_REQUEST`; inserting `FIXED` fails
      as an invalid enum input. Asserted by `db:check-schema`, which carries the instruction to
      invert the assertion in Phase 14.*
- [x] `docs/architecture/DATA_MODEL.md` documents every table, column, enum and constraint added.
      *§12 gains an "As built" record listing every difference from the plan and the five
      constraints added beyond those §6 names; §1.8 gains corrections C7–C10.*
- [x] D9 ten-point contract satisfied.
