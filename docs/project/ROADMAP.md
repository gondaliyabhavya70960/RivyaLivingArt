...
| E7 | **84 source paths named by a descriptive document do not exist**, across 17 documents | `npm run docs:check-paths` reports them — 2870 concrete paths cited across `docs/`, brace groups expanded and placeholders skipped. The 252 in `docs/project/phases/*.md` are excluded by class: a plan written before the code names paths the code later spelled differently (route groups like `(shell)` arrived after Phase 05's plan), which is drift rather than a false claim | Found while verifying the omission register. Each needs judging one at a time — a rename, a plan that was never built, or a genuine gap — so a blanket fix would be guesswork. The script is deliberately NOT in `npm run check`: a gate that fails the build the hour it lands is a gate somebody deletes by Friday. Wire it with `--strict` once the count is zero |
...
| E9 | **The production build has failed twice today on a database `Gateway Timeout`** | Vercel deployments `4C3AX5T6…` (prerendering `/_not-found`, entity `page section`) and `2c5ks4QU…` (prerendering `/product/[slug]`, entity `category`). Both were followed by a green build on the next commit with no code change that could have fixed them | The build queries the database while prerendering, so every deploy depends on it answering promptly. Not a defect in any one commit, and not something a retry hides: it is a fragility worth a timeout-and-retry or a smaller prerender set |
