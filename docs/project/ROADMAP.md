...
| E9 | **The production build failed three times today on a database `Gateway Timeout`** | Vercel deployments `4C3AX5T6…` (prerendering `/_not-found`, entity `page section`), `2c5ks4QU…` (`/product/[slug]`, entity `category`) and `4zMvg74D…` (`/collections/[slug]`, entity `collection`). Three different routes, three different entities, one cause; each was followed by a green build on the next commit with no code change that could have fixed it | The build queries the database while prerendering CMS routes, so every deploy depends on it answering promptly — and today it did not, roughly one build in three. Not any one commit's defect, and not something a retry hides. **Half-fixed on 2026-09-12, after a fourth failure took down the deploy that was meant to put ten new routes live.** `lib/site/prerender.ts` now wraps all five `generateStaticParams` that read the database: the build no longer fails when the database does not answer — those pages render on request instead, which `dynamicParams` (default `true`, and unavailable only under Cache Components, which is off) already guaranteed was correct. The failure is announced in the build log by route and cause rather than swallowed. Two routes had the narrow version of this guard already, matching PostgREST's missing-table codes only, which is why a `Gateway Timeout` walked straight through it. **What is NOT fixed is the timeout itself** — the database still fails to answer roughly one build in three, and now it costs pre-rendering rather than the deployment. That remains open |
...
| E10 | **`/faq`, `/privacy` and `/terms` have route files but zero seeded sections**, so each answers 404 | `app/(site)/{faq,privacy,terms}/page.tsx` all exist; the `pages` rows exist and are PUBLISHED; `page_sections` holds nothing for any of them. No publish step can fix this — there is no content to publish | Found on 2026-09-12 while publishing the launch routes. `/privacy` and `/terms` are legal copy, so BR-H2 puts them behind owner verification and only the owner can write them. `/faq` is seeded copy that was never authored; `faqs` is a table, so the band may be intended to read from it rather than from sections — decide which before writing either |
| E11 | **Three homepage merchandising slots are empty, so two bands are quiet** | `HOMEPAGE_SELECTED_WORKS`, `HOMEPAGE_JOURNAL_STRIP` and `STORE_FEATURED_ROW` each hold 0 entries. `minItems: 3` with `fallback: HIDE_SECTION` means the journal strip is absent from `/` entirely, and selected-works renders its editorial fallback | Working as designed, not a defect: these are hand-curated sets, not "latest three". Seven journal articles and 35 products are published and available to curate. The work is Studio curation at `/studio/merchandising/homepage` and `/studio/merchandising/featured`, not code |
| E12 | **Nothing binds a media asset to a section, so the site renders fallback wells despite 250 published assets** | `media_usages` holds **0** rows and `page_sections.media_slot_key` is null on all 53 sections. `products.hero_media_id`, `categories.hero_media_id`, `collections.hero_media_id`, `portfolio_projects.hero_media_id` and `seo_entries.og_media_id` are all 0 too. The only bound media on the site is `journal_articles.cover_media_id` — 10 rows, 7 of them published | **This is the single biggest gap between "live" and "looks finished", and it is editorial, not technical.** `content/seed/media-bindings.ts` is the file it lands in and says so itself: choosing which of 250 assets illustrates which of 53 sections is a judgement across a dozen pages, and the wrong picture beside the wrong words is the quietest form of the fabrication D10 forbids. Publishing the assets (2026-09-12, BR-H4) removed the *other* blocker, so nothing stands in the way of this pass now except the curation itself |
