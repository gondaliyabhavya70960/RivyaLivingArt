...
| 06 | `0022`, `0030` | `0022` narrows EXECUTE on every `public` function to the roles that need it — found by Supabase's security advisor the first time the schema reached a real project (see the file header). A `media_assets` (full column set, **including** the Higgsfield-provenance columns Phase 07 populates); T `media_usages`; enums `media_kind` (fixed), `media_source`. Three named canary assets only — not the 250 |
