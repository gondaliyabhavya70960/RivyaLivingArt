...
| 08 | `0050`–`0052` | T `pages`, `page_sections`, `content_revisions`, `navigation_items`, `global_content`, `seo_entries`, `faqs`; the CMS trigger set; the four `cms_*` SECURITY DEFINER functions. `0051` is the GENERATED policy file, split out for the reason `0021`, `0031` and `0041` were. `0052` is RESERVED and unused: the design first called for a generated `public.has_permission(text)` for the status-transition trigger, then found `has_role(variadic user_role[])` from `0010` already answers the question — the trigger's generated SQL resolves each permission to its role list at generation time, so no new SQL function exists. The number stays allocated rather than reclaimed; renumbering a block after the fact is how two phases end up claiming one number. This row originally allocated `0050` alone |
