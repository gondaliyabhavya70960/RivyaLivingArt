...
| `research_products.price_state` | `FIXED · STARTING_FROM · REQUEST_QUOTE · PRICE_ON_REQUEST · UNKNOWN` (Phase 28). The first four mirror `products.price_state` character for character so a Phase 31 chart putting a Rivya row beside a research row never begins with a translation somebody would eventually get wrong in one direction. `UNKNOWN` is this table's alone: a first-party product always has a decided posture, and a page Rivya could not read has not decided anything. A check rather than an enum because it is one column's vocabulary and reusing `price_state` would tie a competitor's schema to Rivya's |
| `research_products.dimension_parse_state` | `PARSED · AMBIGUOUS · UNPARSED · ABSENT` (Phase 28), FEAT §23's four. `AMBIGUOUS` and `UNPARSED` are first-class and visible, and the invariant that matters is stated in A28: **`dimensions_mm` is null unless this reads `PARSED`** — every downstream reader reads that column and none of them can tell a guessed millimetre from a read one |
| `research_products.availability` | `IN_STOCK · MADE_TO_ORDER · PREORDER · SOLD_OUT · UNKNOWN` (Phase 28) |
| `research_products.match_method` | `MAP · KEYWORD · MANUAL` (Phase 28). How a row reached its category: a staff-authored mapping row, an exact keyword match on the category's own name (recorded at confidence 0.6, because it is the weaker claim), or a person's hand |
| `research_validation_issues.severity` | `ERROR · WARNING · INFO` (Phase 28). An `ERROR` blocks promotion past `VALIDATED`; the row is retained and listed, never dropped. `blocks` is stored separately from `severity` in `lib/scraper/validation/rules.ts` because "serious" and "holds a row back" are different decisions that happen to agree today |
| `research_match_candidates.method` | `EXTERNAL_ID · TITLE_PRICE · TRIGRAM_DIMENSION` (Phase 28), strongest evidence first. Only the third is a heuristic, and only the third demands positive dimension agreement to auto-merge |
| `research_match_candidates.decided` | `PENDING · ACCEPTED · REJECTED` (Phase 28) |
...
erDiagram
  ...
  research_products ||--o{ research_validation_issues : "unique (product, version, rule, field)"
  research_product_versions ||--o{ research_validation_issues : "the version judged"
  research_products ||--o{ research_match_candidates : "a proposal, never a verdict"
  research_products }o--o| categories : "matched_category_id — crossing 2 of 2 (A28)"
  research_products |o--o| research_products : "duplicate_of_id (set null, reversible)"
...
### 11.x Analytics and comparison — Phase 31 · migrations `0290`–`0292`

| Table | Posture | Written by | Key rule |
|---|---|---|---|
| `research_comparison_sets` | RLS-STAFF (`research.read` select; `research.write` insert/update/delete) | a person, as the session | `edges_match_rule`: a FIXED rule carries edges and a QUANTILE rule carries none |
| `research_comparison_members` | same as its set | a person | one target per row: `(member_type = 'SOURCE') = (source_id is not null)` and the mirror for products |
| `research_analytics_snapshots` | RLS-SERVICE (`research.read` select; **no session write**) | `snapshotScope()` only | `currency_matches_family`: a price snapshot names a currency, the other two families never do; unique per (scope, family, currency, moment) |
| `research_metric_coverage` | RLS-SERVICE, with its snapshot | `snapshotScope()` only | `coverage_pct` generated from `n` / `denominator`; `n <= denominator`; unique per (snapshot, metric key) |

`scope_id` is a bare uuid: one column that names a set, a source or a category cannot reference
three tables, so the cascade a SET scope needs is `tg_research_comparison_set_prune_snapshots()`
instead. No table in this phase references anything outside `research_*`, `research_runs` and
`auth.users`; the I1 allowlist still holds exactly two constraints.

### 11.y The opportunity engine — Phase 32 · migrations `0300`–`0302`

| Table | Posture | Written by | Key rule |
|---|---|---|---|
| `research_scoring_models` | RLS-STAFF (`research.read` select; `research.score.manage` insert/update/delete — owner, admin) | a person | `weights_total = 100`; one ACTIVE; `freeze_active_scoring_model()` refuses a definition change on a non-draft row |
| `research_opportunity_scores` | RLS-SERVICE (`research.read` select; no session write) | `scoreScope()` only | `scored_has_score`; `analytics_snapshot_id` traces the band inputs to a Phase 31 snapshot |
| `research_opportunity_components` | RLS-SERVICE, with its score | `scoreScope()` only | `included = (normalised is not null)`; an excluded row carries a reason |

### 11.z Visual similarity — Phase 33 · migrations `0310`, `0311`, `0313` (`0312` unused)

| Table | Posture | Written by | Key rule |
|---|---|---|---|
| `research_image_hashes` | RLS-SERVICE (`research.read` select; no session write) | nobody — under amendment A33 no competitor byte is fetched | `unique (research_product_id, source_image_key)`; `checksum` indexed, NOT unique |
| `research_similarity_runs` | RLS-STAFF (`research.read` select; `research.similarity.run` insert/update — owner, admin, researcher) | the CLI and the Studio self-check | `(status = 'RUNNING') = (finished_at is null)`; a `MEDIA_ASSET` scope stores no pairs |
| `research_similarity_pairs` | RLS-SERVICE | nobody today | `left_hash_id < right_hash_id`; distance ≤ 18; band is a function of distance, held at the table |
| `research_similarity_suppressions` | RLS-STAFF (`research.write` insert/update/delete) | a researcher | non-blank reason; `created_by not null … on delete restrict` |
| `media_asset_hashes` | **FIRST-PARTY** (`media.read` select; no session write) | the upload path and `npm run media:hash`, service role | `media_asset_id unique`; `(kind = 'IMAGE') = (phash is not null)`; `checksum` NOT unique; never referenced by, or referencing, a research table (I1) |

`research_sources.image_hashing_enabled` (boolean, default false) is gated on `policy_status =
'APPROVED'` exactly as `is_enabled` is, and is set by nothing.

### 11.aa The direction brief — Phase 34 · migrations `0320`–`0321`

| Table | Posture | Written by | Key rule |
|---|---|---|---|
| `research_direction_briefs` | RLS-STAFF (`research.read` select; `research.direction.write` insert/update — owner, admin, merchandiser, researcher; no delete) | a person | `status` CHECKed to four values, never PUBLISHED; `target_category_slug` a checked slug, not a key; `(status = 'APPROVED') = (approved_at is not null)`; `guard_direction_brief_approval()` requires `research.direction.approve` to enter APPROVED; no price, dimension, material or lead-time column |
| `research_direction_brief_evidence` | RLS-STAFF (`research.direction.write` insert/update/delete) | a person, with a reason | `rationale` non-empty; a volatile kind carries a non-empty `captured`; `evidence_id` has no key because one of its seven targets is public |
| `research_direction_brief_revisions` | RLS-SERVICE (`research.read` select; no session write) | `write_direction_brief_revision()` (SECURITY DEFINER) | one snapshot per mutation, numbered per brief under an advisory lock; restore through `research_restore_brief_revision()`, which re-checks the write permission and never restores the status |

§11's earlier row for these tables says `target_category_id uuid references categories(id)`; the
phase document and this section say a checked slug, and the phase document wins (A34).

### 11.ab Shortlist and confirmation — Phase 35 · migrations `0330`–`0331`

| Table | Posture | Written by | Key rule |
|---|---|---|---|
| `research_shortlist_entries` | RLS-STAFF (`research.read` select; `research.confirm` insert/update — owner, admin, merchandiser; no delete) | a person, through `review-actions.ts` or the bulk engine | `reason` non-blank; `captured` an object (score, confidence, model version at entry); one OPEN entry per row (`research_shortlist_entries_one_open_idx`); `closed_at`, `closed_reason`, `closed_by` all three or none; never deleted |
| `research_confirmations` | RLS-STAFF (`research.read` select; `research.confirm` insert/update; no delete) | a person; `created_product_id` and its two stamps by the service role from the bridge only | `decision_note` non-blank; one UNARCHIVED decision per row (`research_confirmations_one_live_idx`); `(archived_at is null) = (archived_reason is null)`; `created_product_id` / `product_started_at` / `product_started_by` all three or none; **no foreign key on `created_product_id`** — resolved only by `resolveStartedProducts` in the research repository |

**Functions.** `guard_research_stage_writer()` — BEFORE UPDATE on `research_products`, raises on a
change to `stage` or `disposition` unless `current_setting('rivya.stage_transition', true) = 'on'`;
defines no transitions. `research_write_stage(p_id, p_stage, p_disposition, p_actor)` — SECURITY
DEFINER, EXECUTE to `service_role` only; sets the flag, updates whichever column is not null, clears
it. It exists because PostgREST gives the repository no transaction of its own; `stage.ts` remains
the only caller of the repository writers that reach it.
...
| 28 | `0260`–`0261` | A `research_products` — twenty-three normalisation columns (`title_normalized`, `brand_text`, `currency`, `price_state`, `price_min_minor`/`price_max_minor`, `dimensions_mm`, `dimension_parse_state`, `material_tokens`, `availability`, `lead_time_days_min`/`_max`, `variant_count`, `image_urls`, `category_labels`, `matched_category_id`, `match_confidence`, `match_method`, `duplicate_of_id`, `normalized_overrides`, `override_by`, `override_at`) and `research_products_matched_category_fk`, **the SECOND AND FINAL boundary crossing the isolation guard's allowlist admits** (`on delete set null`, a taxonomy pointer and never a content pointer — §11, I1); T `research_validation_issues`, `research_match_candidates`, `research_material_lexicon` (forty seeded material terms under `allow-insert`, vocabulary rather than content); F `is_sane_research_dimensions(jsonb)` — an IMMUTABLE predicate mirroring `is_valid_dimensions()` from `0130`, because `research_dimensions_sane` needs a per-key range test and **a CHECK constraint may not contain a subquery**, which the phase document's illustrative SQL does; `refresh_research_search_document()` replaced whole so the `research_product` branch titles a row from its normalised title, falling back to the latest version's raw title, then the external id, then the URL; `tg_research_version_search()` + `research_product_versions_search_sync`, so a new version reindexes its product rather than leaving the row titled by whatever the first fetch said. **`0261` is one past the phase document's `0260`, for the reason A23 gives for `0214`, A24 for `0221`, A25 for `0233`, A26 for `0241` and A27 for `0251`: a generated policy file is rewritten whole by `auth:gen-policies` and cannot also carry the DDL that creates its tables. Two of its three tables are UPDATE-ONLY for every role — rows are written by the pipeline under the service role and a human may only triage what it found — amendment A28** |
| 28·fix | `0262` | Corrects two CHECK constraints in `0260` that passed what they were written to refuse, both the NULL-evaluates-to-PASS family the same file already documents once: `research_material_lexicon_token_shape` was case-INSENSITIVE because `token` is `citext` and citext overloads `~` to `texticregexeq` (fixed by casting `token::text`, which resolves the case-sensitive operator), and `has_no_blank_pattern` let a SQL NULL element through because `btrim(null) = ''` is NULL rather than TRUE. Forward-only: `0260` is applied to both databases and `db:migrate` refuses an edited migration, so the fix is its own number. No data was affected — the table holds only `0260`'s forty seeded rows |
| 29 | `0270`–`0271` | T `research_changes`, `research_change_rules`, `research_review_actions`, `research_notes`, `research_tags`, `research_product_tags`, `research_change_digests`; F `tg_research_review_actions_append_only()`, `tg_research_notes_append_only()` — two BEFORE UPDATE OR DELETE triggers that refuse every UPDATE except setting `undone_by_action_id` / `superseded_by` once from null, and refuse DELETE outright, because a decision log its author can rewrite answers "what does the log say" rather than "what happened". `research_change_rules` uses **`unique nulls not distinct (source_id, field)`** — the default `nulls distinct` would permit any number of global default rows for one field, and the threshold in effect would then depend on row order, which is a bug that cannot be reproduced on demand (PostgreSQL 15+; local 16, hosted 17). Eleven global default rules are seeded under `allow-insert` for the reason `0260`'s lexicon is: a detector with no thresholds classifies nothing. **`0271` is one past the phase document's `0270`, for the reason A23 gives for `0214`, A24 for `0221`, A25 for `0233`, A26 for `0241`, A27 for `0251` and A28 for `0261`: a generated policy file is rewritten whole by `auth:gen-policies` and cannot also carry the DDL that creates its tables. Two of its seven tables take no write policy of any kind — what the detector found is not something a session may author — amendment A29** |
| 30 | `0280`–`0281` | A `research_products` — `scale_band`, **`is_large_format boolean` (NULLABLE, three-valued on purpose)**, `longest_axis_mm`, `large_format_source`, `classified_at`, `classified_rule_id`, plus `research_products_unknown_is_not_large` and `research_products_classification_is_attributed`; T `research_large_format_rules` (five ordered rules seeded under `allow-insert`, first match wins, `unique (priority)`), `research_saved_views` (`unique (owner_user_id, surface, name)`, owner-scoped with a second SELECT leg for shared views). **`is_large_format` is three-valued and that is the phase's central decision**: a boolean would force a row whose dimensions could not be parsed to `false`, and every distribution built on the column would then under-report large work in proportion to how badly a source writes its pages — a failure that looks like a finding. `longest_axis_mm` is written by the classifier rather than generated by the database, because a generated column cannot be written and an editor override has to be possible. **`0281` is one past the phase document's `0280`, for the reason A23 gives for `0214`, A24 for `0221`, A25 for `0233`, A26 for `0241`, A27 for `0251`, A28 for `0261` and A29 for `0271`: a generated policy file is rewritten whole by `auth:gen-policies` and cannot also carry the DDL that creates its tables. `research_saved_views` is the first research table with an `ownerScope`, and its shared leg is a second SELECT policy rather than a widened scope — amendment A30** |
| 31 | `0290`–`0292` | T `research_comparison_sets` (`unique (slug)`, `band_rule` + `band_edges` tied by `edges_match_rule`, edges ascending by `is_strictly_ascending_bigint_array()`), `research_comparison_members` (exactly one target by two CHECKs, `unique nulls not distinct (set_id, member_type, source_id, research_product_id)`), `research_analytics_snapshots` (`currency` is part of the unique key and REQUIRED on a `PRICE_ARCHITECTURE` row — there is no key for a combined-currency figure; `scope_id` carries no foreign key because it points at a set, a source or a category), `research_metric_coverage` (`coverage_pct` GENERATED, `n <= denominator`); F `is_strictly_ascending_bigint_array(bigint[])` (IMMUTABLE, because a CHECK may not hold a subquery), `tg_research_comparison_set_prune_snapshots()` (SECURITY DEFINER, so a deleted set takes its snapshot history with it without a session delete policy on snapshots). **`0291` is the generated RLS file, one past the document's `0290`, for the reason A23 gives for `0214` and every phase since; `0292` holds the indexes the document names as their own deliverable. Snapshots and coverage take no session write policy of any kind — the CLI, the cron and the Studio recompute action write them through the service role — amendment A31** |
| 32 | `0300`–`0302` | T `research_scoring_models` (`unique (version)`, `check (weights_total = 100)`, lifecycle allowlist, activation/retirement dated by CHECK, **one ACTIVE by partial unique index**, `created_by` NULLABLE because `0302` seeds v1 as configuration), `research_opportunity_scores` (`score` nullable only for the state that has no terms, `scored_has_score`, unique per (product, model, moment), index `(model_id, state, score desc)`), `research_opportunity_components` (`included = (normalised is not null)` — excluded is not zero — and `excluded_has_reason`); F `freeze_active_scoring_model()` verbatim from the phase document, BEFORE UPDATE. **`0301` is the generated RLS file, one past the document's `0300`, for the reason A23 gives for `0214` and every phase since; `0302` seeds model v1 as a DRAFT under `allow-insert` — configuration a human activates, never content. Scores and components take no session write policy — amendment A32** |
| 33 | `0310`–`0313` | T `research_image_hashes` (`checksum` indexed and NOT unique; `unique (research_product_id, source_image_key)`; index on `substring(phash from 1 for 16)`), `research_similarity_runs` (a `MEDIA_ASSET` scope stores no pairs, by CHECK), `research_similarity_pairs` (`left_hash_id < right_hash_id`; the band is a function of the distance and the table holds the function; nothing beyond 18), `research_similarity_suppressions`; enum `similarity_band`; `research_sources.image_hashing_enabled` gated on `policy_status = 'APPROVED'`. `0311` creates **`media_asset_hashes`**, a FIRST-PARTY table in its own file (`media_asset_id unique`, `(kind = 'IMAGE') = (phash is not null)`, `checksum` not unique). **`0312` (`research_image_embeddings`, `pgvector`) is NOT USED: embeddings are unreachable while `advanced_similarity` is false and the owner's decision (A33) fetches no competitor bytes to embed; the number stays allocated and unapplied. `0313` is the generated RLS file — amendment A33** |
| 34 | `0320`–`0321` | T `research_direction_briefs` (`status content_status` CHECKed to `DRAFT · REVIEW · APPROVED · ARCHIVED` — PUBLISHED unreachable; **`target_category_slug text` CHECKed to D3's seven, NOT a foreign key** — the phase document over §11's row, open question 13, amendment A34; `(status = 'APPROVED') = (approved_at is not null)`; no price, dimension, material or lead-time column), `research_direction_brief_evidence` (`unique (brief_id, evidence_type, evidence_id)`; non-empty `rationale`; `evidence_id` carries no key because one of the seven targets is public; a volatile kind must carry a non-empty `captured`), `research_direction_brief_revisions` (`unique (brief_id, revision)`); F `write_direction_brief_revision()` (SECURITY DEFINER, after Phase 08's), `research_restore_brief_revision()` (re-checks the write permission; restores prose and category, never the status), `guard_direction_brief_approval()` (APPROVED needs `research.direction.approve`). **`0321` is the generated RLS file — amendment A34** |
| 35 | `0330`–`0331` | **No enum change and no second transition log** — the phase document over this row's earlier text (`ARCHIVED_DECISION`, `research_pipeline_transitions`): archival is a column on the decision record and `research_pipeline_events` stays the only log (amendment A35). T `research_shortlist_entries` (non-empty `reason`; one OPEN entry per row by partial unique index; closed, never deleted), `research_confirmations` (non-empty `decision_note`; one UNARCHIVED decision per row; `created_product_id` with **no foreign key**; `(archived_at is null) = (archived_reason is null)`); F `guard_research_stage_writer()` verbatim from the phase document, BEFORE UPDATE on `research_products`, and `research_write_stage()` (SECURITY DEFINER, service role only) which sets the transaction-local flag and writes in one transaction because PostgREST gives the repository no transaction of its own. `0332` unused. `0331` is the generated RLS file |
