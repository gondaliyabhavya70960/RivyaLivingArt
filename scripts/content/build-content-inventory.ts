...
import { buildVerificationBacklog } from '../../lib/cms/verification-backlog'
...
/**
 * THE ONE WRITER OF THIS DOCUMENT — exported in Phase 46, for the reason the export exists.
 *
 * The file has three generated sections now (the inventory, SEO coverage, the owner-verification
 * backlog) and is regenerated wholesale, so a second script appending a fourth would have its work
 * erased by the next run of this one — silently, in the document Phase 46 hands to the owner.
 * `scripts/content/build-verification-report.ts` therefore calls THIS function rather than writing
 * the file itself, and both entry points produce identical bytes in either order.
 */
export async function writeContentInventory(): Promise<number> {
  ...
  // Phase 46: the owner-verification backlog, from the same database, in the same run — so the
  // count the owner reads on `/studio` and the rows listed here describe one moment in one database.
  const verificationBacklog = await buildVerificationBacklog(client)
  await client.end()
  ...
  const md = [
     ...
     * block so its status is machine-readable, and `scripts/docs/audit-docs.mjs` fails on one that
     * does not. This file is rewritten wholesale on every run, so a block added by hand would
     * survive until the next `npm run content:inventory` and no further — which is exactly how a
     * generated document ends up the only one outside a contract everything else obeys. It was.
     *
     * `owner_verification: OWNER_VERIFICATION_REQUIRED` is not decoration here. This document's
     * whole subject is the rows the owner has not yet confirmed, so the front matter says the same
     * thing the body does.
     */
    '---',
    'doc: INITIAL_CONTENT_INVENTORY',
    'status: CURRENT',
    'owning_phase: 09',
  ...
  // Phase 39: the SEO coverage section, from the same database, in the same run. Phase 46: and the
  // verification backlog after it, each separated by exactly one blank line whatever they end with.
  writeFileSync(
    OUT,
    md.replace(/\n*$/, '\n\n') +
      seoCoverage.replace(/\n*$/, '\n\n') +
      verificationBacklog.replace(/\n*$/, '\n'),
  )
  console.log(
...
// RUN ONLY WHEN RUN. `scripts/content/build-verification-report.ts` imports
// `writeContentInventory` from here, and a module that generated the document as an import SIDE
// EFFECT would write it twice per run — once before the importing entry point had done anything at
// all. The same guard, for the same reason, as `scripts/seo/build-seo-coverage.ts`.
if (process.argv[1]?.endsWith('build-content-inventory.ts') === true) {
  writeContentInventory()
