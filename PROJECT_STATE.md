...
> Last verified: the Rivya UI Redesign, brief phases 1-2 of 6, 2026-09-13 — **PARTIAL**; brief
> phases 3-6 are not done and `docs/SESSION-STATE.md` lists them in order. Phase 45's audit
> questions 1, 5 and 8 remain the owner's; see `docs/design/DESIGN_SYSTEM.md` §19.
...
drive them** — a forward-only migration runner, and **44 gates** that fail the build on the
...
| 01 | PRD, Architecture & Documentation | **COMPLETE — two of its own exit criteria were closed by Phase 46** | `docs/architecture/CANONICAL-DECISIONS.md`, `ARCHITECTURE.md`, `DATA_MODEL.md`, `SCRAPER.md`; `docs/project/PRD.md`, `BUSINESS_RULES.md`, `ROADMAP.md`, `phases/`; `docs/ops/*`; session-recovery file set. **Front matter was missing from sixteen documents its own verification step 4 names, and `ROADMAP.md` shipped without the `Status` column its scope specifies; Phase 46 added both and `scripts/docs/audit-docs.mjs` now fails the build on either.** |
...
| 43 | Media Coverage + Higgsfield Finalization | **DEVELOPMENT COMPLETE** — tests deferred to Phase 42 | Migrations `0410`–`0411`: `media_crops`, one editor-chosen crop per (asset, D6 ratio), applied as `c_crop` BEFORE the delivery preset. The coverage report resolves all 26 declared slots — 15 reuse, 6 re-crop, 3 empty, **2 generate** — after correcting six `fillableBy` mappings and two dispositions the registry had left stale against Phase 07's own analysis. All 250 imported alt-text drafts rewritten from prompt fragments into sentences, in a committed file the generator will not overwrite — **63 of which did not in fact pass the SEED §43 rules and were rewritten again under A63 (2026-09-14)**, because the rules had no test for a BRIEF's vocabulary as opposed to a camera's, and none for an alternative that calls a concept render a photograph. The values are committed; `media_assets.alt_text` still carries the imported drafts until `media:rewrite-alt-text --apply` is run. Studio: Coverage and Concept Placement tabs, the crop editor, the alt-text queue, the brand-format panel. `media:register-external` takes an owner-generated image by Cloudinary URL and refuses any other origin. **Nothing was generated**, and brand marks, 3D models and portfolio imagery never will be. |
...
| 45 | Final Creative Polish | **PARTIAL** — the audit is run and recorded; three questions are the owner's and are outstanding | No migration. `DESIGN_SYSTEM.md` §19 records all eight FEAT §49 questions with evidence, a closed verdict vocabulary, and a front-matter interlock that keeps questions 1, 5 and 8 visible while unanswered; `TESTING.md` §14 carries the five-task walkthrough, the owner's ten Studio operations and the record's shape. **Nine audit dimensions, nine FAILs.** Fixed here: `ProductCard`'s hover transition named a token that does not exist (`--rv-motion-medium`), so every card snapped; three journal surfaces used `rv-container`, which is not a utility and produced no CSS; seventeen `sizes` strings declared a 640px breakpoint this system does not have; and the chrome offered ten destinations that answered 404 — `lib/site/live-paths.ts` composes the gate a category listing actually passes and `lib/site/menu.ts` omits what does not resolve, taking the chrome from 19 destinations to 9, all 200. New gate `design:check-sizes`. **The deferral list was then worked through, 2026-09-12 (#62–#66).** All six conversion affordances exist, four of them added here with a Studio publish control for a category alongside. The hero has a floor — `--rv-hero-min-h: 76svh` through a new `AspectBox` `minBlockSize`, a floor and not a height — and viewport share measured in a browser at the eight widths went from `91·69·61·49·37·91·82·76%` to `91·76·76·76·76·91·82·76%` on `/` and `/large-format`. Section rhythm is `SECTION_RHYTHM`, exhaustive over all 34 block types at four steps. `media_crops` is read: `cropsForAssets` had no callers anywhere, and `loadPageMedia` now resolves crops in one query per page through `MediaSlot` into `MediaImage`'s `src` and every `srcSet` rung. Every band gained a scroll-driven entrance at zero island cost (`app/styles/motion.css`), and two islands left every public route (`HeroMotion`, `MaterialSequence` to `next/dynamic`; budget 7 → 5). Ten media slots bound from the documented family map. Two more gates: `design:check-motion-tokens` and `design:check-token-usage`, plus `content:classify-copy` deliberately outside `check`. The seven `null` renderers were then built (A45), taking the block catalogue to **34 BUILT / 0 PLANNED** — `rich-text` as a plain-text prose band, which leaves A14's refusal of a markup block intact; `faq-list` through `lib/cms/references.ts` rendering native `<details>` at zero island cost, with the band `/faq` had never had; `contact-details` and the footer sharing `ContactChannels` (RC-244). That last one surfaced a site-wide defect nothing had reached: `buildDirectContactUrl` threw on a missing `NEXT_PUBLIC_WHATSAPP_NUMBER`, and the footer calls it on every page as soon as the contact section is published — so publishing it in an environment without the variable answered 500 on every route. It returns null now. The twelve declared-but-unbuilt layout variants were then built, with `ContentCarousel` (RC-222, PLANNED since Phase 16) among them: eleven blocks had shipped two `layoutVariants` each and branched on neither, so the Studio's picker changed nothing and `hero`'s `contained` and `split` produced byte-identical output. `components/sections/CardLayout.tsx` is the one chooser six of them share; the carousel's scroller is a Server Component and its arrows arrive through `next/dynamic`, so the public island budget is unchanged at 5. **Still deferred:** CLS 0.98–1.00 on `/collection` under a cold throttled load, which needs the deployed preview; and the Studio's own §11.1/§13/§15 gaps. **Still the owner's:** questions 1, 5 and 8. |
| 46 | Documentation + Handoff | **PARTIAL — the gates and the backlog are built; four items are the owner’s** | `scripts/docs/audit-docs.mjs` (D7 completeness, front matter, stubs, the D8 secret scan, phase honesty, and `--claims`: the fabrication vocabulary classified by prohibition marker — 424 hits inside prohibitions, 0 assertions); `check-doc-contract.mjs --claims` (53 capability claims, each naming an artefact that resolves, proved by counter-example); `scripts/content/build-verification-report.ts` → the generated backlog of **41 rows across 15 surfaces** on the canonical seeded database, which also fills preflight gate 13; `lib/cms/verification-backlog.ts` + `lib/supabase/repositories/verifications.ts`; front matter on all 34 documents under `docs/` (19 had none) and the viewer taught to drop it; `ROADMAP.md` gained the `Status` column for all 47 phases; recoverability measured from a clean clone at ~6 minutes with the two omitted setup steps recorded in `README.md`. **Outstanding, and only the owner can close them:** the capability-boundary dry run (Phase 45 Q8), the handover session, the runbook screenshots, and the 41-row backlog. | Specified in `docs/project/phases/`. |
| UI&nbsp;Redesign | Rivya UI Redesign against the supplied visual reference — **brief phases 1-2 of 6** | **PARTIAL — the foundation and the public shell are done; brief phases 3, 4, 5 and 6 are not** | Amendments **A46, A47, A48**; the brief itself is now `docs/requirements/03-UI-REDESIGN-BRIEF.md`, verbatim and read-only. **The reference was read, not guessed** — `mcp__Vercel__web_fetch_vercel_url` returned its HTML and all three stylesheets, and its own `:root` names `--mineral:#f4f1e9`, `--sand:#e7e0d5`, `--obsidian:#080a0e` and `clamp(3rem, 7vw + .5rem, 7.5rem)` = 103.41px at 1363px. **The missing-media diagnosis is a data fact, not a code defect:** Cloudinary holds all 252 assets and `media_assets` holds 250 PUBLISHED/VERIFIED rows, but on the hosted project `page_sections.media_desktop_id` is NULL on all 83 rows and `media_usages` has 0 rows, and the cause is the seed runner's own idempotency guard, not a stale run. Rule 5c treats a row published beyond what its module asked for as a human's work and skips it whole; section modules seed DRAFT and every live environment has published them, so the guard skips every visible section including its media columns. Measured by running it: `skipped (owner edit) 35`, `inserted 0`. **An earlier version of this row said the fix was to re-run the binding. That was wrong** — on hosted it would skip all 60 PUBLISHED sections and bind only the 23 DRAFT ones, which nobody sees. The fix has to fill the two media columns only where they are NULL, which overwrites nothing, since a NULL binding is an absence rather than an editor's choice. Built: two warm schemes (MINERAL, SAND) on new primitives rather than on a repointing of `--rv-color-bone`, which was tested and refused because it re-derives 9 of 10 ramp steps; Instrument Serif as the display face; a ground rhythm decided by the page (`components/sections/rhythm.ts`) rather than by each renderer, which is what fixed the homepage rendering 11,993px of a single colour at 1440px; a masthead that overlays a dark opening band through `:has()` and no JavaScript; `MediaSlot.delivery` and four new slots (a registry change — **the homepage still renders zero images on any database**, for the binding reason above, and no claim to the contrary should be made from the seed file alone); **RC-245 `SectionRail`**, a page-section index at `xl` and above costing no island; a **transform-only** band entrance, because the scroll-driven `opacity` animation had made contrast a function of scroll position and failed axe at SERIOUS. Verified: **44/44 gates, 3,083 unit tests, 1,979 browser assertions across four shards at eight widths, 0 failed**; contrast 60 pairs across 5 schemes; island budget unchanged at 5. **Not done:** the remaining public routes, the Studio's 82 routes (blocked on an auth server — 156 specs skip), the per-component external evaluation, visual baselines and Lighthouse. |
